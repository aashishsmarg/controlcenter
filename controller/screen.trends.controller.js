...
router.get('/screentrends' , authMiddleware , async(req , res)=> {
    let result , v_siteid 
    try{
        ...
        res.render('advertising_analytics/screen_trends', { title: "Screen Trends", data: result, username: username, userrole: userrole, v_siteid: v_siteid });
    }catch(er){
        logger.error(`Error in screen.trends.controller - GET method : ${er}`)
    }
})

router.get('/screentrends/reportdata/(:id)/(:fromdate)/(:todate)' , async(req , res) => {
    ...
    try{
        ...
        logger.error(`Error in screen.trends.controller.js Report generation : ${er}`)
    }
